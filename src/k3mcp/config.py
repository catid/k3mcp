...
class Settings:
    ...
    def from_env(cls, env: Mapping[str, str] | None = None) -> Settings:
        ...
        retries = _positive_int(source, "K3MCP_MAX_ATTEMPTS", 3) - 1
        return cls(
            api_key=api_key,
            model=model,
            base_url=base_url.rstrip("/"),
            max_tokens=_positive_int(source, "K3MCP_MAX_TOKENS", 16_000),
            max_input_chars=_positive_int(source, "K3MCP_MAX_INPUT_CHARS", 400_000),
            timeout_seconds=_positive_float(source, "K3MCP_TIMEOUT_SECONDS", 600.0),
            max_retries=retries,
